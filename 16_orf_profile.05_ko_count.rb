...
#--------------------------------------------------
# Program: 16_orf_profile.05_ko_count.rb
...
require 'inifile'
require 'optparse'
h_params = ARGV.getopts('', 'date:20180916', 'kegg_db:kegg_prokaryotes')
...
# scripts
script_pathway_ko_ = "#{dir_scripts_}/kegg_pathway_ko_count.orf_coverage.R"
script_module_ko_ = "#{dir_scripts_}/kegg_module_ko_count.orf_coverage.R"
script_ko_ = "#{dir_scripts_}/kegg_ko_count.orf_coverage.R"
...
inifile.each_section do |section|
	...
	# run script
	system("R --vanilla --slave --args #{in_pathway_} #{dir_out_} #{samples} #{date} #{db_label} <  #{script_pathway_ko_}")
	...
	# run script
	system("R --vanilla --slave --args #{in_module_} #{dir_out_} #{samples} #{date} #{db_label} <  #{script_module_ko_}")

	#------------------------------
	puts "start KO"
	# run script
	system("R --vanilla --slave --args #{in_pathway_} #{dir_out_} #{samples} #{date} #{db_label} <  #{script_ko_}")
