...
for(thre in v.thre){
	...
	df.z <- cbind(v.pathway, v.ko, df.0_1)
	...
	for(pathway in v.pathway.uniq){
		...
		write.table(df.out, file=paste(dir_out_each_,"/sample.kegg_ko.",pathway,".0_1.txt",sep=""), row.names=F, col.names=T, quote=F, sep="\t")
		write.table(df.out.def, file=paste(dir_out_each_,"/sample.kegg_ko.",pathway,".0_1.definition.txt",sep=""), row.names=F, col.names=T, quote=F, sep="\t")
	}
	
	# calculate ratio
	df.ko_ratio <- data.frame(matrix(rep(NA, 1 + length(v.samples)), nrow=1))[numeric(0), ]
	for(pathway in v.pathway.uniq){
		...
		df.ko_ratio <- rbind(df.ko_ratio, c(pathway, v.ratio_ko))
	}
	colnames(df.ko_ratio) <- c("pathway", v.samples)
	write.table(df.ko_ratio, file=paste(dir_out_,"/",db_label,".pathway.ko_ratio.orf_",thre,".txt",sep=""), row.names=F, col.names=T, quote=F, sep="\t")
}
