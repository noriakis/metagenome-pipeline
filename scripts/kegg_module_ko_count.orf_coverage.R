...
for(thre in v.thre){
	...
	df.z <- cbind(v.module, v.ko, df.y[,v.samples])
	...
	for(module in v.module.uniq){
		...
		write.table(df.out, file=paste(dir_out_each_,"/sample.kegg_ko.",module,".count.txt",sep=""), row.names=F, col.names=T, quote=F, sep="\t")
		write.table(df.out.def, file=paste(dir_out_each_,"/sample.kegg_ko.",module,".count.definition.txt",sep=""), row.names=F, col.names=T, quote=F, sep="\t")
	}
	...
	for(module in v.module.uniq){
		...
		mx.ko_count <- rbind(mx.ko_count, v.count_ko)
	}
	df.ko_count <- cbind(v.module.uniq, mx.ko_count)
	colnames(df.ko_count) <- c("module", v.samples)
	write.table(df.ko_count, file=paste(dir_out_,"/",db_label,".module.ko_count.orf_",thre,".txt",sep=""), row.names=F, col.names=T, quote=F, sep="\t")
}
